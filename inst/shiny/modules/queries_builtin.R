...
# the ui function
queries_builtin_ui <- function(id) {
  ns <- NS(id)
  ui <- fluidPage(
    tags$script("
    ...
    wellPanel(
      tags$h3('3. Run the Query'),
      actionButton(ns('run_query'), 'Run Query')
    ),
    wellPanel(
      tags$h3('4. View/Save Results'),
...
# the server function
queries_builtin_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    qry_result <- eventReactive(input$run_query, {
