...
dictionary <- DBI::dbGetQuery(
  ...
  WHERE pk_tablename = 'equipment'
  ORDER by sort_order;")

equipment <- DBI::dbReadTable(conx, "equipment")
equipmodels <- DBI::dbReadTable(conx, "equipmodels")
esettingnames <- DBI::dbReadTable(conx, "esettingnames")
econfignames <- DBI::dbReadTable(conx, "econfignames")
econfigvalues <- DBI::dbReadTable(conx, "econfigvalues")

accounts <- DBI::dbReadTable(conx, "accounts")
